...
"""
The provided code implements the **Aho-Corasick algorithm**, which is a powerful pattern matching algorithm used to efficiently
search for **multiple patterns** in a single pass through the text. It builds an automaton (a finite state machine with failure
links) based on the patterns and then uses it to scan the text.
...
If you're at `'hers'` and the next char doesn’t match, you follow the `fail` link. If that node has an output (like `he`),
